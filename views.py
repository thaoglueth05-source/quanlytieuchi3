...
def user_dashboard(request):
    ...
    if request.method == 'POST':
        ...
        if title and fromDate and toDate and totalUserAllocated:
            try:
                ...
                new_event = Event.objects.create(
                    ...
                    totalUserAllocated=int(totalUserAllocated),
                    totalAmount=cleanAmount or 0,
                    is_adhoc=True,
                ...
                messages.success(request, "Sự kiện đã được gửi đến Admin để duyệt!")
    ...
    # Sự kiện của user này
    my_pending_events = Event.objects.filter(
        is_adhoc=True,
        approval_status=EventApprovalStatus.PENDING
    ).order_by('-id')

    my_rejected_events = Event.objects.filter(
        is_adhoc=True,
    ...
    context = {
        ...
        'my_rejected_events': my_rejected_events,
